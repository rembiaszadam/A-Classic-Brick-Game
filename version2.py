...
class MyGame(arcade.Window):
    ...
    def pad_return(self, ball):
        ...
        # Calculate ball speed, needed to back calculate x-vector after bounce.
        ball_speed = math.sqrt((ball.change_x ** 2) + (ball.change_y ** 2))
        # Pad surface simulates a sin wave from -60 to +60 deg.
        pad_angle = math.asin((ball.center_x - self.paddle.center_x) / PADDLE_WIDTH)
        # Ball angle before hitting the pad.
        ball_angle = math.atan(ball.change_y / ball.change_x)

        angle_change = math.fabs((2 * ball_angle) + (2 * pad_angle)) - PI

        #  Set ball direction based on ball angle and calculated angle change.
        if ball_angle < 0:
            new_ball_angle = ball_angle + angle_change
            ndx = -1
        else:
            new_ball_angle = ball_angle - angle_change
            ndx = 1

        if new_ball_angle < -PI or new_ball_angle > PI:
            ndy = -1
        else:
            ndy = 1

        # Keep y-velocity constant.
        ball.change_y = BALL_DY * ndy
        # Set x-velocity based on ball angle and y-velocity.
        ball.change_x = ball_speed * math.cos(math.fabs(new_ball_angle)) * ndx
    ...
    def brick_hit(self, brick, ball):
        ...
        # Set the direction of x and y velocity based on which corner is hit.
            ...
            # Rebound the ball based on a reflection angle of 45°.
            # i.e. new change in x is old change in y and vice versa.
            new_change_x = BALL_DY * dx
            new_change_y = math.fabs(ball.change_x) * dy

            ball.change_y = new_change_y
            ball.change_x = new_change_x
